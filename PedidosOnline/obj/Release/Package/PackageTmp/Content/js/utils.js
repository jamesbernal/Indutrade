...
var PANEL_ANIMATION_DELAY = 13 /*ms*/
...
function setUpPanels()
{
	...
	for (var i=0; i<headingTags.length; i++)
	{
		...
		el.parentNode.className = PANEL_COLLAPSED_CLASS; //PANEL_NORMAL_CLASS;
		...
		else
		{
		    ...
		    panelsStatus[name] = "false"; //(el.parentNode.className == PANEL_NORMAL_CLASS) ? "true" : "false";
		}
		*/
...
{
	// the "proper" way	
	window.addEventListener("load", setUpPanels, true);
	collapseAll();
}
else 
...
function LoadView(urlToNavigate) {
    ...
    if (urlToNavigate.indexOf("/SGIC/Informe") != -1) {
...
function LoadViewContent(urlToNavigate) {
    //Incluir icono de cargando
